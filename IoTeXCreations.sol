...
contract IoTeXCreations is Wrap, Ownable, Pausable, CloneFactory, WhitelistAdminRole {
	using SafeMath for uint256;
	...
	address payable public feeAddress = address(0x547003547C9510b5B03Dfd64aA7E710600af09A8);
    uint256 public farmFee = 100;
    uint256 public farmFeeMinimumIotx = 100 * 10**18;
	...
	function redeem(address erc1155Address, uint256 id) external payable updateReward(msg.sender) {
        ...
        require(msg.value == fees, "Send the proper IOTX for the fees");
	 ...
	 * */
	 function init(
	    ...
	    require(!constructed && !isCloned, "IoTeXCreations must not be constructed yet or cloned.");
	    require(_minStake >= 0 && _maxStake > 0 && _maxStake >= _minStake, "Problem with min and max stake setup");
	 ...
	 function newFarm(
	    ...
	    IoTeXCreations(clone).init(_periodStart, _minStake, _maxStake, _controller, _tokenAddress, _uri, msg.sender);
	    IoTeXCreations(clone).setCloned();
	    IoTeXCreations(clone).addWhitelistAdmin(msg.sender);
	    IoTeXCreations(clone).addPauser(msg.sender);
	    IoTeXCreations(clone).renounceWhitelistAdmin();
	    IoTeXCreations(clone).renouncePauser();
	    IoTeXCreations(clone).transferOwnership(msg.sender);

	    farms[msg.sender].push(clone);

	    // enough IOTX or a wildcard? then there won't be no fee
	    if(iotxBalance < farmFeeMinimumIotx && !needsPayment()){
	        feeAddress.transfer(msg.value);
	    }

	    emit FarmCreated(msg.sender, clone, iotxBalance < farmFeeMinimumIotx && !needsPayment() ? farmFee : 0, _uri);
    ...
    */
	...
	function setFarmFeeMinimumIotx(uint256 _minIotx) external onlyWhitelistAdmin{
	    require(!isCloned, "Not callable from clone");
	    farmFeeMinimumIotx = _minIotx;
	}
